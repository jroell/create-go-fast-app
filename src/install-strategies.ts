...
export function getInstallStrategy(packageManager: string, config: ProjectConfig): InstallStrategy {
  const baseEnv = {
    ...process.env,
...
function getInstallFixInstructions(packageManager: string, errorMessage: string): string {
...
function getGeneralInstallFixInstructions(packageManager: string): string {
...
function getClearCacheCommand(packageManager: string): string {
  switch (packageManager) {
...
function getVerboseCommand(packageManager: string): string {
  switch (packageManager) {
...
export function validatePackageManager(packageManager: string): boolean {
  const validManagers = ['npm', 'yarn', 'pnpm', 'bun'];
  return validManagers.includes(packageManager);
}

export async function checkPackageManagerVersion(packageManager: string): Promise<{
  version: string;
  compatible: boolean;
