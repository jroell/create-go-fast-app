...
export async function performSystemChecks(config: ProjectConfig): Promise<SystemCheckResult> {
...
function checkPackageManager(packageManager: string): SystemCheck {
  try {
    let version: string;
    
    switch (packageManager) {
      case 'npm':
        version = execSync('npm --version', { encoding: 'utf8', stdio: 'pipe' }).trim();
        break;
      case 'yarn':
        version = execSync('yarn --version', { encoding: 'utf8', stdio: 'pipe' }).trim();
        break;
      case 'pnpm':
        version = execSync('pnpm --version', { encoding: 'utf8', stdio: 'pipe' }).trim();
        break;
      case 'bun':
    ...
    return {
      ...
      fixInstructions: installInstructions[packageManager as keyof typeof installInstructions],
      severity: 'error'
    };
  }
}
...
async function checkNetworkConnectivity(): Promise<SystemCheck> {
  try {
...
function checkDiskSpace(projectName: string): SystemCheck {
  try {
...
function checkDirectoryPermissions(projectName: string): SystemCheck {
  try {
    const targetPath = resolve(process.cwd(), projectName);
    ...
    try {
      ...
      return {
        ...
        fixInstructions: platform() === 'win32' 
...
function checkWindowsPathLength(projectName: string): SystemCheck {
  if (platform() !== 'win32') {
    return {
      name: 'Path Length',
      passed: true,
...
function checkBuildTools(): SystemCheck {
  try {
    ...
    try {
      pythonVersion = execSync('python --version', { encoding: 'utf8', stdio: 'pipe' }).trim();
      pythonCheck = true;
    } catch {
      try {
        pythonVersion = execSync('python3 --version', { encoding: 'utf8', stdio: 'pipe' }).trim();
        pythonCheck = true;
      } catch {
        // Python not found
      }
...
async function checkRegistryAccess(packageManager: string): Promise<SystemCheck> {
  try {
...
export function displaySystemChecks(result: SystemCheckResult): void {
